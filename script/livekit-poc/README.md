...
- 7882/UDP：WebRTC 媒体；
- macOS / Windows：当前 `docker-compose.yml` 走端口映射模式，webhook URL 用 `host.docker.internal:48090` 让容器访问到宿主机 aiptower 后端；
...
- Linux：可以把 `docker-compose.yml` 改成 `network_mode: host` + 删 `ports:` 段，并把 `livekit.yaml` 的 webhook URL 改为 `http://127.0.0.1:48090/admin-api/im/livekit/webhook`。
