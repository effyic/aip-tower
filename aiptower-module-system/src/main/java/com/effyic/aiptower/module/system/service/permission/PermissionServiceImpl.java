package com.effyic.aiptower.module.system.service.permission;
...
import com.effyic.aiptower.framework.common.biz.system.permission.dto.DeptDataPermissionRespDTO;
import com.effyic.aiptower.framework.security.core.util.SecurityFrameworkUtils;
...
import com.effyic.aiptower.module.system.dal.dataobject.permission.RoleMenuDO;
import com.effyic.aiptower.module.system.dal.dataobject.permission.UserRoleDO;
import com.effyic.aiptower.module.system.dal.mysql.permission.RoleMenuMapper;
import com.effyic.aiptower.module.system.dal.mysql.permission.UserRoleMapper;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public void assignUserRole(Long userId, Set<Long> roleIds) {
        ...
        // 计算新增和删除的角色编号
        Set<Long> roleIdList = new HashSet<>(CollUtil.emptyIfNull(roleIds));
        // 非超管：不能新分配超级管理员；前端不可见时会漏传，保留用户已有超管角色
        if (!isCurrentUserSuperAdmin()) {
            Set<Long> dbSuperAdminRoleIds = filterSuperAdminRoleIds(dbRoleIds);
            roleIdList.removeAll(filterSuperAdminRoleIds(roleIdList)); // 禁止新分配
            roleIdList.addAll(dbSuperAdminRoleIds); // 禁止误删
        }
    ...
    public Map<Long, Set<Long>> getUserRoleIdListByUserIds(Collection<Long> userIds) {
        ...
        return CollectionUtils.convertMultiMap2(userRoleMapper.selectListByUserIds(userIds),
    ...
    private boolean isCurrentUserSuperAdmin() {
        ...
        return roleService.hasAnySuperAdmin(getSelf().getUserRoleIdListByUserIdFromCache(userId));
    }

    /**
     * 从角色编号中筛出超级管理员角色
     */
    private Set<Long> filterSuperAdminRoleIds(Collection<Long> roleIds) {
        if (CollUtil.isEmpty(roleIds)) {
            return Collections.emptySet();
        }
        return convertSet(roleIds, roleId -> roleService.hasAnySuperAdmin(Collections.singleton(roleId)) ? roleId : null);
    }
